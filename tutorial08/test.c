...
static void test_access_object() {
    ...
    for (i = 0; i < 8; i++) {
        char key[] = "a";
        key[0] += i + 1;
        /*size_t index = lept_find_object_index(&o, key, 1);
        const lept_value* v = lept_get_object_value(&o, index);
        double num = lept_get_number(v, index);
        EXPECT_EQ_DOUBLE((double)i + 1, num);*/
